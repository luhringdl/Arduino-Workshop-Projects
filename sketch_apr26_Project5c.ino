// Project 5c - Controlling Traffic by Levi Luhring
...
int flowTime = 4000;          // Time for traffic flow
...
void loop() {
  ...
  if (digitalRead(westButton) == LOW && !trafficWest) {
    allowTrafficFlow(true);  // Switch to westbound traffic
  }
  if (digitalRead(eastButton) == LOW && trafficWest) {
